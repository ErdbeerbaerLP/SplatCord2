...
public class SplatnetOrderThread extends TimerTask {
    @Override
    public void run() {
        try {
                    ...
                    if (orders.size() > 0) {
                        ...
                        for (Order o : orders) {
                            if ((g.gear.__splatoon3ink_id).equals(o.gear)) {
                                ...
                                final MessageCreateBuilder b = new MessageCreateBuilder();
                                b.addContent(lang.botLocale.cmdSplatnetOrderFinished.replace("%ping%", user.getAsMention()));
                                final EmbedBuilder emb = new EmbedBuilder().setTimestamp(Instant.ofEpochSecond(g.getEndTime())).setFooter(lang.botLocale.footer_ends).setThumbnail(g.gear.image.url).setAuthor(lang.s3locales.gear.get(g.gear.__splatoon3ink_id).name + " (" + lang.s3locales.brands.get(g.gear.brand.id).name + ")", null, g.gear.brand.image.url);
                                final StringBuilder sb = new StringBuilder();
                                for (Power p : g.gear.additionalGearPowers) {
                                    sb.append(Emote.resolveFromS3Ability(p.__splatoon3ink_id));
                                }
                                emb.addField(lang.botLocale.skillSlots, Emote.resolveFromS3Ability(g.gear.primaryGearPower.__splatoon3ink_id).toString() + sb, true);
